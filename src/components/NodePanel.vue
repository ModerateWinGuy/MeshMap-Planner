        ...
        <template v-if="store.nodes.length || store.groups.length">
            ...
            >
                <div
                    ...
                    :class="{ 'drop-into': isOver({ kind: 'into-group', id: group.id }), dragging: isGroupDragSource(group.id) }"
                ...
                >
                    ...
                    />
                    <template v-else>
                        <span class="flex-grow-1 text-truncate fw-medium" @dblclick.stop="startRename(group)">{{ group.name }}</span>
                        <span class="badge bg-secondary-subtle text-secondary-emphasis rounded-pill flex-shrink-0">{{ nodesInGroup(group.id).length }}</span>
                    </template>
                ...
                >
                    ...
                    <li
                        ...
                        class="list-group-item text-muted small fst-italic empty-folder"
                    >Drag nodes here</li>
                </ul>
            </div>
            ...
            <div
                v-if="hasGroups"
                class="ungrouped-header text-muted small text-uppercase fw-semibold px-1 mb-1 mt-2"
            >Ungrouped</div>
            <ul
                class="list-group ungrouped"
                :class="{ 'drop-zone-active': hasGroups && dragKind === 'node', 'drop-into': isOver({ kind: 'ungrouped' }) }"
