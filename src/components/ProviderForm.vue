<template>
    <form novalidate class="d-flex flex-column gap-2 py-1">
        ...
        <div class="d-flex align-items-center gap-2">
            ...
            <span v-if="testResult" :class="testResult.ok ? 'text-success' : 'text-danger'" class="small">{{ testResult.message }}</span>
        </div>
...
const urlTemplateValid = computed(() =>
    localUrlTemplate.value.includes('{z}') && localUrlTemplate.value.includes('{x}') && localUrlTemplate.value.includes('{y}')
)
