<template>
  <div>
    <h2 class="h6 d-flex align-items-center gap-2">
      ...
      <InfoTip>
        Pick a MeshCore <strong>contacts export</strong> (JSON). Repeaters and room servers that
        carry a location are added as nodes in an <strong>Imported</strong> folder; companions and
        contacts without a location are skipped. You can review and deselect before importing.
      </InfoTip>
    </h2>
    ...
    <template v-if="parsed">
      <div class="d-flex align-items-center justify-content-between mt-3 mb-1">
        <span class="small fw-semibold">{{ candidates.length }} contact{{ candidates.length === 1 ? '' : 's' }} found</span>
        <div class="d-flex gap-2">
          <button type="button" class="btn btn-outline-secondary btn-sm py-0 px-1" :disabled="allSelected" @click="setAll(true)">All</button>
          <button type="button" class="btn btn-outline-secondary btn-sm py-0 px-1" :disabled="noneSelected" @click="setAll(false)">None</button>
        </div>
      </div>
      <ul class="list-group">
        <li v-for="(c, i) in candidates" :key="i" class="list-group-item d-flex align-items-start gap-2 py-1">
          ...
          <span v-if="c.isDuplicate" class="badge bg-warning-subtle text-warning-emphasis rounded-pill flex-shrink-0 align-self-center" title="A node with this name and location already exists">duplicate</span>
        </li>
      </ul>
      ...
      <div class="d-flex gap-2 mt-2">
        <button type="button" class="btn btn-success btn-sm w-100" :disabled="!selectedCount" @click="confirmImport">Import {{ selectedCount }} selected</button>
