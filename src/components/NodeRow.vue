<template>
    <li
        ...
        :class="{ active: node.id === store.selectedNode?.id, dragging: isDragSource, 'drop-before': showDropLine }"
        role="button"
        draggable="true"
    ...
    >
        ...
        <span class="d-flex align-items-center gap-2 flex-shrink-0">
            <button
                ...
                :title="inHiddenFolder ? 'Folder is hidden — show the folder to see this node' : (node.hidden ? 'Show on map' : 'Hide from map')"
