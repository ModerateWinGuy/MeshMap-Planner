    ...
    <template v-else-if="store.profileResult">
      ...
      <div v-if="chart" class="profile-chart">
        ...
        >
          ...
          <line
            v-for="(mx, i) in chart.xMinorTicks"
            :key="'xm' + i"
            :x1="mx"
            :y1="VB_H - PAD_B"
            :x2="mx"
          ...
          <g v-for="t in chart.xTicks" :key="'x' + t.v">
            ...
            <text :x="t.x" :y="VB_H - PAD_B + 24" text-anchor="middle" class="axis-label">
              {{ t.v }}
            </text>
          </g>
          ...
          <path
            v-if="chart.fresnel60Line"
            :d="chart.fresnel60Line"
            fill="none"
            stroke="#ff8a8a"
            stroke-width="1.5"
...
const linkPayload = (): SharePayload | null => {
  ...
  return {
    v: 1,
    t: 'link',
    n: [nodeToShared(a), nodeToShared(b)],
    lp: store.splatParams.lora?.preset,
  };
};
...
const fromCoord = computed(() =>
  fromNode.value
    ? `${fromNode.value.transmitter.tx_lat.toFixed(4)}, ${fromNode.value.transmitter.tx_lon.toFixed(4)}`
    : '',
);
...
function onChartClick(evt: MouseEvent) {
  ...
  const [lng, lat] = interpGreatCircle(
    a.transmitter.tx_lon,
    a.transmitter.tx_lat,
    b.transmitter.tx_lon,
    b.transmitter.tx_lat,
    f,
  );
