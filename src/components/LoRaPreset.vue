...
const spreadingFactor = computed({
  ...
  set: (value: number) => {
    store.splatParams.lora.spreadingFactor = value;
    syncPresetLabel();
  },
});
const bandwidthKhz = computed({
  ...
  set: (value: number) => {
    store.splatParams.lora.bandwidthKhz = value;
    syncPresetLabel();
  },
});
...
const presetName = computed({
  get: () => {
    ...
    if (
      savedPreset &&
      savedPreset.spreadingFactor === spreadingFactor.value &&
      savedPreset.bandwidthKhz === bandwidthKhz.value
    ) {
      return saved!;
    }
