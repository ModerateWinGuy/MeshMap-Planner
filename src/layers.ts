...
// are larger and orange with a glow; unselected ones are a smaller red marker. Colour drives the SVG
// stroke via currentColor; the drop-shadow keeps the monotone outline legible over busy basemaps.
export function stylePinElement(el: HTMLElement, selected: boolean, name: string): void {
  ...
  // bright orange (matches the brand accent), unselected = red — both on the warm theme.
  el.style.color = selected ? '#fd7e14' : '#dc3545';
}

export function makePinElement(selected: boolean, name: string): HTMLElement {
  ...
  el.appendChild(svg);
  stylePinElement(el, selected, name);
  return el;
}
