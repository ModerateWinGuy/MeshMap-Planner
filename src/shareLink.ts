...
// to a hidden textarea + execCommand, then a prompt, so a copy still works in odd contexts. Exported
// standalone (not just via useShareLink) so plain-text copies (e.g. "copy coordinates") don't need to
// go through the share-link/payload machinery.
